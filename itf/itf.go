...
func main() {
	...
	binPath := path.Join(workDir, "wut4.out")
	...
	disassembledSourcePath := path.Join(workDir, "wut4.dsm")
	err = runDisassembler(binPath, disassembledSourcePath)
	if err != nil {
		fatal(fmt.Sprintf("dsm: %s: %s", binPath, err.Error()))
	}
	pr(fmt.Sprintf("disassembled %s to %s", binPath, disassembledSourcePath))

	reassembledBinPath := path.Join(workDir, "wut4.out2")
...
const Disassembler string = "../dsm/dsm"
