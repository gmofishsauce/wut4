...
void U1_rising_edge(void) {
    ...
    if (GetPOR()) {
        ...
        SET1(B1_state, 2, not4s(getnet(U2_8)));
        SET1(B1_state, 3, not4s(getnet(U2_11)));
    }
    setbus(B1, B1_SIZE, GETN(B1_state, B1, B1_SIZE));
}

void N8_U2_3_clock_is_high(void) {
    setnet(U2_3, xor4s(GetVCC(), getnet(B1+0)));
}

void N9_U2_6_clock_is_high(void) {
    setnet(U2_6, xor4s(getnet(U2_3), getnet(B1+1)));
}

void N10_U2_8_clock_is_high(void) {
    setnet(U2_6, xor4s(getnet(U2_6), getnet(B1+2)));
}

void N11_U2_11_clock_is_high(void) {
    setnet(U2_8, xor4s(getnet(U2_8), getnet(B1+3)));
}
